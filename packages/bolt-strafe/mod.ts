...
import { tocore, tostrafe } from './messages.ts';

export class strafe_plugin extends plugin<{ token: string }> {
	bot: Client;
	name = 'bolt-strafe';
	...
	async create_bridge(channel: string) {
		...
		if (!ch.havePermission('Sudo')) {
			throw new Error('Please enable Sudo permissions!');
		}
		return ch.id;
	}
	...
	async edit_message(
		...
		await message.edit(await tostrafe({ ...msg, reply_id }));
		return edit_id;
	}
